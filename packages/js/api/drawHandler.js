...
const initHandler = function (DrawMode, clampMode) {
    let handler, clampmode = 0;
    if (Cesium.defined(clampMode)) clampmode = clampMode;
    switch (DrawMode) {
        case "Polyline":
            handler = new Cesium.DrawHandler(viewer, Cesium.DrawMode.Line, clampmode);
            break;
        case "Polygon":
            handler = new Cesium.DrawHandler(viewer, Cesium.DrawMode.Polygon, clampmode);
            break;
    };
    // 半透线创建
    ...
    handler.handlerDrawing = function () {
        return new Promise((resolve, reject) => {
            let clearActFn = handler.activeEvt.addEventListener((isActive) => {
                if (isActive == true) {
            ...
            let clearMovFn = handler.movingEvt.addEventListener((windowPosition) => {
            ...
            let clearDrawFn = handler.drawEvt.addEventListener((result) => {
